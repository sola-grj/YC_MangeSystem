...
class Customer(models.Model):
    ...
    def show_classes(self):
        ret = ' | '.join([str(i) for i in self.class_list.all()])
        return ret
    ...
    def show_status(self):
        ...
        color = static_dic.get(self.status)
        des = self.get_status_display()
        return '<span style="background-color: {};padding:5px">{}</span>'.format(color,des)
...
class ClassList(models.Model):
    ...
    def show_teachers(self):
        ret = '|'.join([str(i) for i in self.teachers.all()])
        return ret
...
class ConsultRecord(models.Model):
    ...
    def __str__(self):
        return "%s-%s" %(self.customer.qq,self.customer.name)
